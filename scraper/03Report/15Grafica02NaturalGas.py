...
# Cargar el archivo CSV ubicado en "./historicalDatabase/02NaturalGasCashHistoricalSemana.csv"
csv_path = "scraper/01HistoricalDatabase/02NaturalGasCashHistoricalSemana.csv"
...
# Crear la figura y la gráfica de línea en color rojo con el label "$/mmbtu"
plt.figure(figsize=(10, 5.5))
plt.plot(df['Time'], df['Last'], color='red', label='$/mmbtu')

# Configurar los intervalos del eje Y: de 0 a 10 en pasos de 1
...
for idx in year_change_indices:
    ...
    plt.text(df.loc[idx, 'Time'], 8, str(df.loc[idx, 'Time'].year),
    ...
    plt.annotate(f"({df.loc[idx, 'Time'].strftime('%m/%d/%y')}) {df.loc[idx, 'Last']}", 
                 ...
                 xytext=(df.loc[idx, 'Time'], df.loc[idx, 'Last'] - 1),
...
if len(df) > 0:
    plt.annotate(f"({df.iloc[0]['Time'].strftime('%m/%d/%y')}) {df.iloc[0]['Last']}", 
                 ...
                 xytext=(df.iloc[0]['Time'], df.iloc[0]['Last'] + 0.5),
...
if len(df) > 0:
    plt.annotate(f"({df.iloc[-1]['Time'].strftime('%m/%d/%y')}) {df.iloc[-1]['Last']}", 
                 ...
                 xytext=(df.iloc[-1]['Time'], df.iloc[-1]['Last'] + 0.5),
                 arrowprops=dict(facecolor='black', arrowstyle='->'),
                 fontsize=9)
...
# Agregar el texto "barchart.com \n NGY00" en la esquina inferior derecha de la gráfica
